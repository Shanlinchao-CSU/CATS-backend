...
public class RegisterApplicationServiceImpl implements RegisterApplicationService {
    ...
    public CommonResponse<String> EnterpriseUserRegister(File file ,String account_name ,String password ,String phone ,String email ,int enterprise_type ,int type ){
        ...
        RegisterApplication registerApplication1 = new RegisterApplication(account_name,password,phone,email,type,str,enterprise_type);
    ...
    public CommonResponse<String> ThirdPartyRegulatorsRegister(File file ,String account_name ,String password ,String phone ,String email ,int type ){
        RegisterApplication registerApplication = registerApplicationMapper.selectOne(new QueryWrapper<RegisterApplication>().eq("phone", phone).eq("email", email));
        String str= String.valueOf(file);
        //100为企业类型之外的编号
        RegisterApplication registerApplication1 = new RegisterApplication(account_name,password,phone,email,type,str,100);
        if(registerApplication!=null){
            return CommonResponse.createForError();
        }else {
            int i = registerApplicationMapper.insert(registerApplication1);
